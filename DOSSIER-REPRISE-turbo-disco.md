...
**Polish HUD (2026-07-27)** : retrait des boutons on/off du son (accueil, pseudo, jeu) — la musique reste active, sans bouton, pour désencombrer. Retrait de la jauge « TOUR % » + barre de progression (haut-droite). Consigne de départ corrigée : « POSE TON DOIGT SUR LA PISTE » (plus de boule disco). Nouvelle **barre haute** semi-opaque (comme celle du bas) : chrono à gauche, **RECORD** à droite, lisibles par-dessus les crabes. Bouton « → PARTAGER » : le lien pointe désormais vers l'écran-titre `/turbo-disco/` (et non plus directement dans le jeu via `?go=1`).

**Écran Lanterne Rouge (2026-07-27)** : nouvelle cérémonie dédiée au « Dernier Mondial » (le tour le plus lent du championnat, `rank===total && total>3`). Écran rouge `#sLantern` (titre 🏮, pseudo, temps, message honorifique) avec **collecte de l'insta** (`updateInsta` → colonne `insta` de la ligne de score) au cas où l'asso offre un lot de consolation. Boutons Rejouer / Scores (retour géré via `lbFrom='lantern'`). Le reset auto est suspendu tant que la cérémonie est affichée. Ne se déclenche qu'au-delà de 10 scores (avant, tout le monde passe par l'écran résultat top-10).
